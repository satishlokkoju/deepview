...
from deepview.introspectors._dim_reduction._dimension_reduction import (
    DimensionReduction,
...
from deepview._availability import _umap_available
...
_logger = logging.getLogger("deepview.introspectors.DatasetReport")
...
def _projection_default() -> t.Optional[OneOrManyDimStrategies]:
    if not _umap_available():
        _logger.warning("UMAP not available, not running projection in report."
                        "To fix, install deepview['dimreduction'].")
        return None
...
class ReportConfig:
    ...
    def n_stages(self) -> int:
        """
        How many stages of :func:`multi introspect <deepview.base.multi_introspect>` need to be
