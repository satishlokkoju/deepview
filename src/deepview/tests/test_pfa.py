...
# Some tests use TF and deepview_tensorflow, only run those if TF and deepview_tensorflow are installed
try:
...
from deepview.introspectors._pfa._pfa_units import _get_corr_and_inactive_units, _DirectionalStrategy
from deepview.exceptions import DeepViewException
from deepview.samples import (
    StubImageDataset,
    StubProducer
)
...
@pytest.mark.skipif(not _tensorflow_available(),
                    reason="TensorFlow and/or deepview_tensorflow not installed.")
class TestPFAforCNN:
    ...
    def test_pfa_on_keras_model(self) -> None:
        ...
        with pytest.raises(DeepViewException):
