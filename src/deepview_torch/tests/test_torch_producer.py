...
def _run_producer(metadata: t.Sequence[t.Any], mapping: t.Sequence[t.Any]) -> Batch:
    ...
    producer = deepview_torch.TorchProducer(loader, mapping)
...
def _run_dataset(fields: t.Optional[t.Mapping[str, np.ndarray]],
    ...
    ds = deepview_torch.ProducerTorchDataset(producer, mapping)
    loader = torch_data.DataLoader(ds, batch_size=2, shuffle=False)
...
def test_transforms() -> None:
    ...
    ds = deepview_torch.ProducerTorchDataset(
...
def test_round_trip_deepview_pycharm_deepview() -> None:
    """
    ...
    producer2 = deepview_torch.TorchProducer(loader, ["image", "image2", key1, key2])
...
def test_example() -> None:
    ...
    _ = deepview_torch.ProducerTorchDataset(
        producer, [
            "image",
            squares,
            "vector",
